...
declare type Operand = vec2 | vec3 | vec4 | Readonly<vec2> | Readonly<vec3> | Readonly<vec4> | number;
declare type Larger<U extends Operand, V extends Operand> = U extends vec4 | Readonly<vec4> ? vec4 : V extends vec4 | Readonly<vec4> ? vec4 : U extends vec3 | Readonly<vec3> ? vec3 : V extends vec3 | Readonly<vec3> ? vec3 : U extends vec2 | Readonly<vec2> ? vec2 : V extends vec2 | Readonly<vec2> ? vec2 : number;
declare type ArithmeticType<U extends Operand, V extends Operand> = Larger<U, V>;
export declare function plus<U extends Operand, V extends Operand>(a: U, b: V, out?: ArithmeticType<U, V> & vec): ArithmeticType<U, V>;
